...
The REST API server for the game, written in PHP.  It uses Laminas to handle the majority of the code.


## Install Future Extensions

To install additional PHP extensions without going through the headaches of installing the PHP requirements, use the included docker:

```bash
$ docker build -t county-php-server-install -f Dockerfile.dev .
$ docker run --rm -it -v $(pwd):/opt/local county-php-server-install
```
